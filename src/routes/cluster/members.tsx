...
import { membersQueryOptions, clusterInfoQueryOptions } from '@/lib/queries/etcd'
import { updateMember, promoteMember, removeMember, moveLeader } from '@/lib/server/etcd'
...
function MembersPage() {
  ...
  const moveLeaderMutation = useMutation({
    mutationFn: (targetMemberId: string) => moveLeader({ data: { targetMemberId } }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['members'] })
  ...
  const openDialog = (member: EtcdMember, dialog: 'update' | 'promote' | 'moveLeader' | 'remove') => {
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => openDialog(member, 'update')}>
                            Update member
                          </DropdownMenuItem>
                          {member.isLearner && (
                          ...
                          {isLeader && (
                            <DropdownMenuItem onClick={() => openDialog(member, 'moveLeader')}>
                              Move leadership
                            </DropdownMenuItem>
                          )}
                          <DropdownMenuItem
