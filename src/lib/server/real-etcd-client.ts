...
export class RealEtcdClient implements EtcdClientInterface {
	...
	async moveLeader(
		...
		return withErrorHandling(async () => {
			...
			await this.client.maintenance.moveLeader({ targetID: targetMemberIdBigInt });
	...
	async defragment(): Promise<{ success: boolean; message: string }> {
		return withErrorHandling(async () => {
			await this.client.maintenance.defragment();
	...
	async compact(
		...
		return withErrorHandling(async () => {
			...
			let compactRevision = revision;
			if (!compactRevision) {
				const status = await this.client.maintenance.status();
				compactRevision = Number(status.header.revision);
			}

			await this.client.kv.compact({ revision: compactRevision });
			return {
				success: true,
				message: `Compaction completed at revision ${compactRevision}`,
			};
		});
	}
	...
	async snapshot(): Promise<{ success: boolean; path: string }> {
		return withErrorHandling(async () => {
			...
			const stream = await this.client.maintenance.snapshot();
