import { EtcdError, EtcdAuthenticationFailedError, EtcdPermissionDeniedError } from 'etcd3';
...
export function handleEtcdError(error: unknown): EtcdErrorResult {
  ...
  if (error instanceof EtcdError) {
    ...
    // Key not found
    if (message.includes('key not found') || message.includes('etcdserver: key not found')) {
      return {
        code: 'NOT_FOUND',
    ...
    // User/role not found
    if (message.includes('user name not found') || message.includes('role name not found')) {
      return {
        code: 'NOT_FOUND',
        message: message,
        isAuthError: false,
