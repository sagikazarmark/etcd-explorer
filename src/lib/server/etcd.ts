...
import { z } from "zod";
import type { Permission } from "etcd3";
import type {
	EtcdKey,
	EtcdClusterInfo,
	EtcdAuthStatus,
...
export const revokeUserRole = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }): Promise<EtcdUser | null> => {
		if (isMockMode()) {
			...
			users = users.map((u, i) =>
				i === userIndex
					? { ...u, roles: u.roles.filter((r) => r !== data.role) }
					: u,
			);
			return users[userIndex];
		}
		...
		return withErrorHandling(async () => {
			...
			if (data.permission.prefix) {
				...
				await role.grant({
					permission: permType,
					range: client.range({
						start: data.permission.key,
						end: data.permission.rangeEnd,
					}),
				});
			} else {
				await role.grant({
				...
				i === roleIndex
					? {
							...
							permissions: r.permissions.filter(
								(p) =>
									!(
										p.key === data.permission.key &&
										p.permType === data.permission.permType
									),
							),
						}
					: r,
		...
		return withErrorHandling(async () => {
			...
			const leasesWithDetails: EtcdLease[] = await Promise.all(
				(leaseList.leases || []).map(async (lease) => {
					...
					try {
						const ttlInfo = await client.leaseClient.leaseTimeToLive({
							ID: leaseId,
							keys: true,
						});
						return {
...
export const updateMember = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }): Promise<EtcdMember | null> => {
		...
		return withErrorHandling(async () => {
			...
			await client.cluster.memberUpdate({
				ID: memberId,
				peerURLs: data.peerURLs,
			});
...
export const compact = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }): Promise<{ success: boolean; message: string }> => {
		...
		return withErrorHandling(async () => {
			...
			return {
				success: true,
				message: `Compaction completed at revision ${revision}`,
			};
		});
	});
